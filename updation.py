...
def training_addn(model,X_train,Y_train,epochs=2,batch_size=128):
...
def evaluation(model,X_test,Y_test):
    ...
    # plt.scatter(np.arange(Y_test.shape[0]),abs(Y_pred[:,0]-Y_test)*10,s=18)
    # plt.show()

    # plt.plot(np.arange(Y_test.shape[0]),Y_pred,'yo-',markersize=5)
    # plt.plot(np.arange(Y_test.shape[0]),Y_test,'bo-',markersize=5)
    # plt.show()

    return RPA*100, Y_pred
...
for name in Name:
    ...
    model = training_addn(model,X_train,Y_train/10)
